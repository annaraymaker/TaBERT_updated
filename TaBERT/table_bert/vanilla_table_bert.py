...
class VanillaTableBert(TableBertModel):
    ...
    def to_tensor_dict(
        ...
        # basic tensors
        input_array = np.zeros((batch_size, max_sequence_len), dtype=np.int32)
        mask_array = np.zeros((batch_size, max_sequence_len), dtype=np.bool_)
        segment_array = np.zeros((batch_size, max_sequence_len), dtype=np.bool_)
        ...
        if table_specific_tensors:
            ...
            context_token_indices = np.zeros((batch_size, max_context_len), dtype=np.int32)
            context_mask = np.zeros((batch_size, max_context_len), dtype=np.bool_)
            column_token_mask = np.zeros((batch_size, max_sequence_len), dtype=np.bool_)

            # we initialize the mapping with the id of last column as the "garbage collection" entry for reduce ops
            column_token_to_column_id = np.zeros((batch_size, max_sequence_len), dtype=np.int32)
            column_token_to_column_id.fill(max_column_num)

            column_mask = np.zeros((batch_size, max_column_num), dtype=np.bool_)
